...
def compute_regression_metrics(eval_pred):
    ...
    return {"mse": mse,"mae": mae,"r2": r2,"spearmanr": spearman_corr}
...
class MLMDataCollator:
    ...
    def __call__(self, examples):
        ...
        indices_mask = torch.bernoulli(torch.full(labels.shape, 0.8,device=device)).bool() & masked_indices
        inputs[indices_mask] = self.mask_token_id
        indices_random = torch.bernoulli(torch.full(labels.shape, 0.5,device=device)).bool() & masked_indices & ~indices_mask
        random_words = torch.randint(self.vocab_size, labels.shape,device=device)
        inputs[indices_random] = random_words[indices_random]
        batch["cds_input_ids"] = inputs
        batch["labels"] = labels
        batch["masked_indices"] = masked_indices
        batch["mask_token_positions"] = indices_mask
        return batch
