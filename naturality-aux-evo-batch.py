...
class GeneticOptimizer:
    ...
    def calculate_perplexity(self, cds_sequences, nat_protein_emb):
        ...
        all_perplexities_by_seed = [[] for _ in range(n_seeds)]
        ...
        for i in range(0, len(cds_sequences),self.batch_size):
            ...
            for j in range(len(batch_sequences)):
                sample = {'cds_input_ids':cds_encoding['input_ids'][j].squeeze(),
                    'cds_attention_mask':cds_encoding['attention_mask'][j].squeeze()}
            ...
            for seed_idx, mlm_collator in enumerate(mlm_collators):
                ...
                with torch.no_grad():
                    ...
                    self.perplexity_model.plantrna.to('cpu')
                    ...
                    for b in range(logits.size(0)):
                        ...
                        if mask_positions.sum() == 0:
                            batch_perplexities.append(1.0)
                            continue
                        masked_logits = sample_logits[mask_positions]
                        masked_labels = sample_labels[mask_positions]
                    ...
                    all_perplexities_by_seed[seed_idx].extend(batch_perplexities)

        avg_perplexities = []
        for seq_idx in range(len(cds_sequences)):
            if all(seq_idx < len(perplexities) for perplexities in all_perplexities_by_seed):
                avg_perplexity = sum(all_perplexities_by_seed[seed_idx][seq_idx] for seed_idx in range(n_seeds)) / n_seeds
    ...
    def get_attention_weights(self, cds_sequence,expr_protein_embs):

        protein_sequence = self.translate_cds(cds_sequence)
        ...
        for key, model in self.models.items():
            ...
            attention_weights = []
            def hook_forward(self, hidden_state,attention_mask):
                ...
                pooled_output = torch.matmul(weights, hidden_state)
                return pooled_output.squeeze(1)


            model.pooler.forward = hook_forward.__get__(model.pooler, type(model.pooler))

            with torch.no_grad():

                model(cds_input_ids=cds_encoding['input_ids'],
        ...
        mean_attention_weights = np.mean(all_attention_weights, axis=0)
    ...
    def predict_expression_batch(self,cds_sequences, expr_protein_embs):
        ...
        for i in range(0, len(cds_sequences),self.batch_size):
            batch_sequences = cds_sequences[i:i + self.batch_size]
            batch_proteins = self.translate_cds(batch_sequences)
            cds_encoding = self.cds_tokenizer(
                batch_sequences,
                padding='max_length',
                truncation=True,
    ...
    def predict_expression(self, cds_sequence,expr_protein_embs):
        if isinstance(cds_sequence, list):
            return self.predict_expression_batch(cds_sequence, expr_protein_embs)
        else:
            result = self.predict_expression_batch([cds_sequence],expr_protein_embs)
            return result[0]

    def calculate_fitness(self, cds_sequences, nat_protein_emb,expr_protein_embs):
        expression_probs = self.predict_expression_batch(cds_sequences, expr_protein_embs)
        perplexities = self.calculate_perplexity(cds_sequences, nat_protein_emb)
        fitness_scores = [expr * (perp ** self.perplexity_weight) for expr, perp in zip(expression_probs,perplexities)]
        return fitness_scores, expression_probs, perplexities

    def create_initial_population(self,cds_sequence,expr_protein_embs):
        attention_weights = self.get_attention_weights(cds_sequence,expr_protein_embs)
        max_length = min(1022,len(cds_sequence) // 3)
        codon_attn_weights = attention_weights[1:(max_length + 1)]
        codon_attn_weights = codon_attn_weights / np.sum(codon_attn_weights)
        hash_value = stable_hash(cds_sequence)
        weights_filename = os.path.join(self.weights_save_path,f"attention_weights{hash_value}.npy")
        np.save(weights_filename,codon_attn_weights)
        print(f"Saved attention weights to {weights_filename}")
        ...
        for i in range(0, max_length):
            ...
            valid_replacements = [c for c in synonymous_codons.get(amino_acid,[])]
            synonymous_options.append(valid_replacements)
        population = [cds_sequence]
        while len(population) < self.population_size:
            mutated_seq = self.mutate(cds_sequence, mutation_probs,synonymous_options)
            population.append(mutated_seq)
        return population, mutation_probs, original_protein, synonymous_options

    def mutate(self, cds_sequence, mutation_probs,synonymous_options=None):
        sequence_list = list(cds_sequence)
        codon_count = len(sequence_list) // 3
        mutation_mask = np.random.random(len(mutation_probs)) < mutation_probs
        for i in np.where(mutation_mask)[0]:
            if synonymous_options and i < len(synonymous_options) and synonymous_options[i]:
                codon_pos = i * 3
                new_codon = random.choice(synonymous_options[i])
                sequence_list[codon_pos:codon_pos + 3] = list(new_codon)
        return ''.join(sequence_list)

    def crossover(self, parent1, parent2):
        if random.random() > self.crossover_rate:
            return parent1, parent2
        crossover_point = random.randint(0,len(parent1) // 3) * 3
        child1 = parent1[:crossover_point] + parent2[crossover_point:]
        child2 = parent2[:crossover_point] + parent1[crossover_point:]
        return child1, child2

    def optimize(self, cds_sequence):
        print("Starting CDS sequence optimization...")
        self.all_sequences_data = {}
        ...
        with torch.no_grad():
            self.perplexity_model.load_state_dict(load_file(f"{self.perplexity_model_dir}/model.safetensors"))
            self.perplexity_model.to(self.device)
            ...
            self.perplexity_model.plantrna.to('cpu')
            nat_protein_emb = nat_protein_emb.to(self.device)
            expr_protein_embs = {}
            for key, model in self.models.items():
                ...
                model.plantrna.to('cpu')
        population, mutation_probs, original_protein, synonymous_options = self.create_initial_population(cds_sequence,expr_protein_embs)
        creation_time = time.time() - start_time
        print(f"Initial population creation time: {creation_time:.2f} seconds")
        start_time = time.time()

        fitness_scores, expression_probs, perplexities = self.calculate_fitness(population, nat_protein_emb,expr_protein_embs)
        evaluation_time = time.time() - start_time
        print(f"Initial population evaluation time: {evaluation_time:.2f} seconds")
        for seq, expr, perp in zip(population,expression_probs,perplexities):
        ...
        best_expression = expression_probs[best_idx]
        best_perplexity = perplexities[best_idx]
        population_with_scores = list(zip(population, fitness_scores,expression_probs, perplexities))
        print(f"Initial best fitness: {best_fitness:.4f}")
        print(f"Initial best expression probability: {best_expression:.4f}")
        print(f"Initial best perplexity: {best_perplexity:.4f}")
        history = {
            'generation': [0],
            ...
            'avg_expression': [np.mean(expression_probs)],
        ...
        for generation in range(1,self.max_generations + 1):
            ...
            num_top = int(self.population_size * self.selection_top_percent)
            top_indices = np.argpartition(fitness_scores, -num_top)[-num_top:]
            ...
            while len(new_population) < self.population_size:
                ...
                child1 = self.mutate(child1,mutation_probs,synonymous_options)
                child2 = self.mutate(child2,mutation_probs,synonymous_options)
                new_population.append(child1)
                if len(new_population) < self.population_size:
                    new_population.append(child2)
            crossover_mutation_time = time.time() - start_crossover_time
            print(f"Crossover and mutation time: {crossover_mutation_time:.2f} seconds")
            population = new_population[:self.population_size]

            start_eval_time = time.time()
            fitness_scores, expression_probs, perplexities = self.calculate_fitness(population, nat_protein_emb, expr_protein_embs)
            evaluation_time = time.time() - start_eval_time
            print(f"Population evaluation time: {evaluation_time:.2f} seconds")
            ...
            current_best_expression = expression_probs[current_best_idx]
            current_best_perplexity = perplexities[current_best_idx]
            population_with_scores = list(zip(population, fitness_scores,expression_probs,perplexities))
            ...
            history['best_expression'].append(best_expression)
            history['best_perplexity'].append(best_perplexity)
            history['avg_fitness'].append(np.mean(fitness_scores))
            history['avg_expression'].append(np.mean(expression_probs))
            history['avg_perplexity'].append(np.mean(perplexities))
            history['all_scores'].append(population_with_scores)

            print(f"Current best fitness: {current_best_fitness:.4f}")
            print(f"Current best expression probability: {current_best_expression:.4f}")
            print(f"Current best perplexity: {current_best_perplexity:.4f}")
            print(f"Historical best fitness: {best_fitness:.4f}")
            if no_improvement_gen >= patience:
                print(f"\nNo improvement for {patience} generations, stopping optimization")
                break

        final_protein = self.translate_cds(best_sequence)
        ...
        original_perp = self.calculate_perplexity([cds_sequence],nat_protein_emb)[0]
        print(f"Original expression probability: {original_expr:.4f}")
        print(f"Original perplexity: {original_perp:.4f}")
        ...
        sorted_sequences = sorted([(seq, data['expression'] * (data['perplexity'] ** self.perplexity_weight)) for seq, data in self.all_sequences_data.items()],
                                  key=lambda x: x[1],
                                  reverse=True)
        top_n = min(self.top_n_return,len(sorted_sequences))
        top_sequences = [seq for seq, _ in sorted_sequences[:top_n]]
        top_expressions = [self.all_sequences_data[seq]['expression'] for seq in top_sequences]
        top_perplexities = [self.all_sequences_data[seq]['perplexity'] for seq in top_sequences]
        top_fitness_scores = [expr * (perp ** self.perplexity_weight) for expr, perp in zip(top_expressions,top_perplexities)]
...
def main():
    ...
    for seq_id, cds_sequence in sequences.items():
        ...
        if args.results:
            seq_results = osp.join(args.results,seq_name,"results.txt")
            os.makedirs(osp.dirname(seq_results),exist_ok=True)
        else:
            seq_results = None

        if args.history:
            seq_history = osp.join(args.history,seq_name,"optimization_history.csv")
            os.makedirs(osp.dirname(seq_history),exist_ok=True)
        else:
            seq_history = None
        ...
        if is_valid_cds(cds_sequence):
            result = optimizer.optimize(cds_sequence)
            optimized_sequences[seq_id] = result
            if seq_history:
                result['history'].to_csv(seq_history, index=False)
            ...
            if seq_results:
                with open(seq_results, 'w') as f:
                    ...
                    for i in range(len(result['top_sequences'])):
                        ...
                        perp = result['top_perplexities'][i]
                        fitness = result['top_fitness_scores'][i]

                        original_seq = result['original_sequence']
                        nucleotide_changes = sum(1 for a, b in zip(original_seq, seq) if a != b)
                        nucleotide_change_rate = nucleotide_changes / len(original_seq)

                        codon_changes = 0
                        for j in range(0,len(original_seq),3):
                            if original_seq[j:j + 3] != seq[j:j + 3]:
                                codon_changes += 1
    ...
    with open(args.output, 'w') as f:
        for seq_id, result in optimized_sequences.items():
            for i in range(len(result['top_sequences'])):
                seq = result['top_sequences'][i]
                expr = result['top_expressions'][i]
                perp = result['top_perplexities'][i]
                fitness = result['top_fitness_scores'][i]
                f.write(f">{seq_id} naturalness={perp:.4f} expression={expr:.4f} fitness={fitness:.4f}\n")
                f.write(f"{seq}\n")
